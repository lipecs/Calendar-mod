...
const saveUser = async () => {
    ...
    showAlert('error', t('Erro ao salvar usuário') + ': ' + (error.response?.data?.message || error.message));
  } finally {
    isLoading.value = false;
  }
};
...
const deleteUser = async (userId) => {
  ...
  try {
    ...
    showAlert('error', t('Erro ao excluir usuário') + ': ' + (error.response?.data?.message || error.message));
  } finally {
    isLoading.value = false;
  }
};
      ...
      >
        ...
        <template #item.role="{ item }">
          <VChip
            ...
            :key="role.name"
            :color="role.name === 'ROLE_ADMIN' ? 'primary' : 'success'"
            size="small"
            class="text-capitalize me-1"
          >
            {{ role.name === 'ROLE_ADMIN' ? $t('Admin') : $t('Usuário') }}
          </VChip>
        </template>
